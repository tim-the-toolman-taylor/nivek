...
--
-- What this does:
--   1. Creates nivek.stalk (one target chatter per channel + last_message).
--   2. Seeds/updates the global builtin !stalk row (handler_key = 'stalk').
...
VALUES (
    ...
    'stalk',
    'everyone',
    'Quotes the last chat message from the chatter this channel is stalking. Anyone can run it; mods/broadcaster pick the target with "!stalk set <username>" (or from the dashboard) and clear it with "!stalk clear".'
)
ON CONFLICT (trigger) WHERE scope = 'global' DO NOTHING;

UPDATE nivek.command
   SET description = 'Quotes the last chat message from the chatter this channel is stalking. Anyone can run it; mods/broadcaster pick the target with "!stalk set <username>" (or from the dashboard) and clear it with "!stalk clear".',
       updated_at = now()
 WHERE trigger = '!stalk'
   AND scope = 'global';
